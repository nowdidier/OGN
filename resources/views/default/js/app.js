...
focusId.forEach(el => el.addEventListener("click", function (e) {
  makeRequest("/focus", { body: "content_id=" + el.dataset.id + "&type=" + el.dataset.type + "&_token=" + token });
}));

// Adding Folders
saveFolder.forEach(el => el.addEventListener("click", function (e) {
  makeRequest("/folder/content/save", { body: "id=" + el.dataset.id + "&type=" + el.dataset.type + "&tid=" + el.dataset.tid + "&_token=" + token });
}));

// Add or remove your post to your profile 
addProfile.forEach(el => el.addEventListener("click", function (e) {
  makeRequest("/post/profile", { body: "post_id=" + el.dataset.post + "&_token=" + token });
}));

// Deleting a linked content folder 
delFolderContent.forEach(el => el.addEventListener("click", function (e) {
  makeRequest("/folder/content/del", { body: "id=" + el.dataset.id + "&type=" + el.dataset.type + "&tid=" + el.dataset.tid + "&_token=" + token });
}));

// Removing a tag
delFolder.forEach(el => el.addEventListener("click", function (e) {
  makeRequest("/folder/del", { body: "id=" + el.dataset.id + "&type=" + el.dataset.type + "&_token=" + token });
}));

// Recommend a post
postRecommend.forEach(el => el.addEventListener("click", function (e) {
  makeRequest("/post/recommend", { body: "post_id=" + el.dataset.id + "&_token=" + token });
}));

// Deleting / restoring content
typeAction.forEach(el => el.addEventListener("click", function (e) {
  makeRequest("/status/action", { body: "content_id=" + el.dataset.id + "&type=" + el.dataset.type + "&_token=" + token });
}));
...
delVotingOption.forEach(el => el.addEventListener("click", function (e) {
  makeRequest("/poll/option/del", { body: "id=" + el.dataset.id + "&_token=" + token });
}));
